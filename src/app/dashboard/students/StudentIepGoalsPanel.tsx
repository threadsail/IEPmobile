...
const textareaClass =
  "w-full min-w-0 rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-900 placeholder-zinc-500 focus:border-pink-500 focus:outline-none focus:ring-1 focus:ring-pink-500 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-100 dark:placeholder-zinc-400 dark:focus:border-pink-400 dark:focus:ring-pink-400";
const selectClass =
  "w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-900 focus:border-pink-500 focus:outline-none focus:ring-1 focus:ring-pink-500 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-100 dark:focus:border-pink-400 dark:focus:ring-pink-400";
...
export default function StudentIepGoalsPanel({ student }: Props) {
  ...
  const fieldDisabled = isArchivedSelection;
  ...
  return (
    <section className="rounded-lg border border-zinc-200/80 bg-zinc-50/50 p-4 dark:border-zinc-700/50 dark:bg-zinc-800/40">
      ...
      <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
        Pick a goal (archived goals show with a tag). Save changes before archiving or unarchiving so
      ...
      <div className="mt-4 rounded-lg border border-zinc-200/80 bg-white/60 p-4 dark:border-zinc-700/40 dark:bg-zinc-900/40">
        ...
        <div className="mt-3">
          <label
            htmlFor={`goal-select-${student.id}`}
            className="mb-1 block text-xs font-medium text-zinc-600 dark:text-zinc-400"
          >
            Goal
          </label>
          <select
            id={`goal-select-${student.id}`}
            className={selectClass}
            value={selectOptionCount === 0 ? "" : selectedKey}
            disabled={selectOptionCount === 0}
            aria-label="Select goal"
            onChange={(e) => {
              const v = e.target.value;
              if (v === "") return;
              if (parseGoalSelectKey(v)) setSelectedKey(v);
            }}
          >
            {selectOptionCount === 0 ? (
              ...
              <>
                {draftActiveGoals.map((row, i) => (
                  <option key={`a-${i}`} value={formatGoalSelectKey("active", i)}>
                    {storedGoalListLabel(serializeGoal(row.title, row.description), i)}
                  </option>
                ))}
                {archivedRows.map((row, i) => (
                  <option key={`z-${i}`} value={formatGoalSelectKey("archived", i)}>
                    {storedGoalListLabel(serializeGoal(row.title, row.description), i)} (Archived)
                  </option>
                ))}
              </>
            )}
        ...
        <div className="mt-4 space-y-3">
          {selectOptionCount === 0 ? (
            <p className="text-sm text-zinc-500 dark:text-zinc-400">
              No goals yet. Use <span className="font-medium">New goal</span> to add a title and
              description.
            </p>
          ) : (
            <>
              <div>
                ...
                <input
                  ...
                  className={`${inputClass} ${fieldDisabled ? "cursor-not-allowed opacity-70" : ""}`}
              ...
              <div>
                ...
                <textarea
                  ...
                  disabled={fieldDisabled}
                  className={`${textareaClass} min-h-[7rem] resize-y ${fieldDisabled ? "cursor-not-allowed opacity-70" : ""}`}
                  placeholder="Measurable details, criteria, or notes for this goal"
                />
              </div>

              <div className="flex flex-wrap items-center gap-2">
                {!isArchivedSelection && !hasContent ? (
                  <button
                    type="button"
          ...
          <form key={student.id} action={saveAction} className="space-y-3 border-t border-zinc-200/80 pt-3 dark:border-zinc-700/50">
            <input type="hidden" name="id" value={student.id} />
